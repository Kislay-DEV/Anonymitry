...
const Feed = () => {
  ...
  const handleLike = async (postId) => {
    try {
      ...
      if (error.response && error.response.status === 400) {
        const response = await axiosInstance.put(`/api/post/unlike/${postId}`);
        setPosts(posts.map(post => post._id === postId ? { ...post, likes: response.data.likes } : post));
      } else {
  ...
  const handleRepost = async (postId) => {
    try {
      const response = await axiosInstance.post(`/api/post/repost/${postId}`);
      setPosts(posts.map(post => post._id === postId ? { ...post, reposts: response.data.reposts } : post));
    } catch (error) {
      if (error.response && error.response.status === 400) {
        const response = await axiosInstance.post(`/api/post/unrepost/${postId}`);
        setPosts(posts.map(post => post._id === postId ? { ...post, reposts: response.data.reposts } : post));
      } else {
            ...
            >
              ...
              <div className="px-4 py-3 bg-slate-600/50 backdrop-blur-sm rounded-b-xl 
                ...
                <div className="flex space-x-6">
                  ...
                  <button onClick={() => handleRepost(post._id)} className="flex items-center space-x-2 text-slate-300 hover:text-blue-400 
                                   transition-all duration-300 transform hover:scale-110">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" 
                            d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
                    </svg>
                    <span className="text-sm font-medium">Repost {post.reposts}</span>
                  </button>
